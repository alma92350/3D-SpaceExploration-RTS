...
| **N-07** | **Documentation**: every architectural choice has an ADR; every phase has exit criteria; the board is current at the end of every session | `npm run check:adr` (**25 ADRs, all indexed with a status**) | It checks that every ADR is indexed, not that every architectural choice has one. The board being current is a habit, not a gate — though the open-questions register having eight rows reading OPEN against settled decisions (P6-T08) is what that habit failing looks like |
...
**Update, and it is the most important line in this file: one of the five has now been run.**
`mvp.md` met a tester on 2026-08-15 against `1208f2ee1f40`. It produced **five findings, none of
which any gate in six phases had caught**, and the largest was that the camera basis was mirrored —
pan inverted on both axes and the minimap a 180° rotation of the battlefield (PT-01, ADR-0025). All
five are fixed; the rows are in `TASKS.md` under *Playtest findings*.

What that does **not** do is close P6-T09, and the reason belongs here rather than in a footnote:
**the tester wrote the game.** S1 and S6 were passed by someone who already knew where everything
was, which is the weakest possible evidence for two gates that exist to measure a first encounter.
`mvp.md` asks for five testers, at least two who have never played the 2D game. This was one who
has. The script's real question is still unanswered.

The session did settle one thing that no amount of engineering could have: **three of the five
reports were about features that already worked**, and the tester could not find them. That is not a
result a test suite can produce, and it is the argument for the remaining four scripts.

**P6-T09: five playtest scripts, one run by its own author and four never run.** `mvp.md`, `economy.md`, `combat.md`,
`galaxy.md` and `longgame.md` are complete and carry gates written in advance so a result cannot be
argued into a pass afterwards. Four have never met a tester; the fifth met its own author, which is
recorded above as the partial result it is.
