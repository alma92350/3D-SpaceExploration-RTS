...
| 47 | **The production menu offers every unit the engine allows** | `queueProduction` gates on `def.odysseyOnly && !state.endless` | `producibleTypes` in `hud.ts` — **`queueProduction`'s own gate, not half of it.** The old menu filtered `def.odysseyOnly` where the engine gates on `def.odysseyOnly && !state.endless`, and every world here is endless, so six of eighteen units were hidden from a menu the engine would have accepted them into — *pinned by a test*, which is why four phases missed it. `prereqsMet` filters as it does for buildings; affordability and supply are shown, never obeyed. `test/ui/production-menu.test.ts` **enumerates rather than lists**: its oracle is `queueProduction` run for real and rolled back, asserted as an iff **per producer over every `UNITS` key**, so a seventh gated unit arrives red on its own — the sweep §7.4 said did not exist. All 17 produced units reachable; the 18th (`freighter`) is computed unreachable because nothing produces it. **The Helium Bomb was the real cost**: P3-T10 built its whole arming, fuse and two-ring blast path, its mesh and its `armBomb`/`detonate` intents for a unit only `aiIndustry.js:241` could ever mint, so a player could be hit by one and never build one. The test now trains one through the button. 21 of 21 mutations caught | PRESENT |
| 48 | **Affordability read across all 23 commodities** | `canAfford` | `canAfford(snap.stockpile, cost)` — the engine's rule over the snapshot's own 23-commodity bag (row 41), so the HUD stays a pure function of the snapshot while the *rule* stays the engine's. The re-derivation knew **three** commodities; the sweep finds **nine** reading a permanent zero, not the five this row named — and the Leviathan and Helium Bomb would have joined them the moment row 47 unhid them, which is why the two rows are one fix. Tested by zeroing each costed commodity in turn and asserting the flag flips for exactly the options needing it, plus equality with `canAfford` at five stockpiles across **both** `production` and `builds`. **Still shown, never hiding**: a struck-through button is asserted to keep its command, because `bind()` only toggles a class on purpose. A mutation that drops affordability from the positional row's build half survived a first draft — `actions` is assembled on two separate lines and needed two assertions | PRESENT |
...
| 93 | **News from a world you are not standing on** | `sweepColonies` returns `{type: "lost" \| "hostile" \| "attacked", planetId}` | `src/ui/news.ts` on the starmap's News board, fed from a drain the shell was **already doing** — **and the previous trace in this cell was wrong**: `takeColonyNotes` has had a caller since `676aa08`, `app/game.ts`'s render loop. What it did not have was anywhere to land. `HudView.notice` is ONE shared element, cleared only by the next notice and shared with command errors, so a colony falling was erased by the next refused order, several notes on one frame showed only the last, and the line then sat there forever because `notice` is called only when there is something to say. The board gives the news memory, a merge window and an age; the count the window folded is kept, because a raid that coalesced into one line contributed one entry and three hundred events. It is **beside** `view/alerts.ts` and not in it, for three reasons and the third is fatal: `AlertFeed` is fed only by the snapshot (`view/` may not import the engine), it is keyed on a world position a colony note does not have, and `Game.adoptSeat()` clears it on every jump — which would delete this news at precisely the moment it is the only thing worth knowing | PRESENT |
| 94 | **Starmap toasts the engine queues for itself** | `galaxy.pacifyNotes`, `expansionNotes`, `milestones` | Same board, and **this cell named three of four queues**: `galaxy.rivalGateNotes` is the fourth, created lazily by `checkRivalGate` and rebuilt by nothing on load — so it is `undefined` on a fresh galaxy *and* a loaded one, and a reader assuming an array throws on the first frame of every session. It carries the ascension row 98 says the starmap stops being able to draw at the worst moment. **Nothing drains any of them.** A cursor reads them, because a drain from `ui/` is a write to a galaxy field (ADR-0008 §3) and would silently empty `ui/milestones-panel.ts`, which reports `galaxy.milestones` as a list. The queues are cumulative since the galaxy was created or loaded, so the cursor is PRIMED on adoption and `since` says when the board started watching — replaying the backlog would open a session by firing every firework of the run at once. `galaxy.reliefNote` is not read at all: it latches, so the edge is `lastReliefTime` changing. 34 of 34 mutations caught | PRESENT |
...
| 104 | **New-game setup: faction and difficulty** | `PLAYABLE_FACTIONS` (3), `factionTrait`, `DIFFICULTY_OPTIONS` (3, each with ~12 tuning fields) | `src/ui/new-game.ts` — the one model in this directory that takes no galaxy, because it decides what one is built from; `worldOptionsFor` fills the two `WorldOptions` fields `main.ts` never passed, spreading over ADR-0010 §2's fixed seed and world so both survive. **Every session ever played was medium / frontier**, including every measurement in `perf/` and every scenario in the four playtest scripts. Rows are `PLAYABLE_FACTIONS` and `DIFFICULTY_OPTIONS` mapped, so an entry added upstream appears without an edit, and traits resolve through `factionTrait` rather than out of `FACTIONS[id].traits`. **Medium's empty column is the trap**: it carries none of the thirteen dials, and defaulting an absent one to 1 is wrong by three for `counterEvery` (Easy sets 0, the use site reads `?? COUNTER_EVERY` = 3), so `present` and `value: null` say "the engine does not name this here". **And the engine validates neither field**, both proved by building galaxies rather than by reading code: an unknown difficulty is stored verbatim and played as Medium, an unknown faction is stored while conferring nothing. The pick reaches the **seat**, not the galaxy (`neighbourAiProfile` resolves every other world), and the model says so. 33 of 33 mutations caught | PRESENT |
...
**This file has now been wrong twice, in both of the ways it warns about.** §4's count was three
mutually inconsistent numbers because nobody had counted (it now carries the command that counts
it), and **row 93's trace asserted that `takeColonyNotes` had no caller in `src/` when it had had one
since `676aa08`** — the row was right that the capability was unreachable and wrong about why, which
sent P5-T16 looking for a missing drain instead of a defective channel. Both were written from
reading rather than from running. That is the failure this file exists to prevent, committed inside
the file itself, and it is the strongest argument in the document for §2's scripts over anybody's
recollection.
