...
| P2-T05 | PARTIAL | BLOCKED | P2-T04 | `hauler`, `heavyhauler`, `bulkfreighter`, `freighter` render; a laden unit is distinguishable from an empty one at camera distance; **cargo adds no draw call** | **Q-11 answered by ADR-0014: one hull, not four.** `hauler`, `heavyhauler`, `bulkfreighter` and `freighter` share the `freighter` mesh — the same silhouette at any distance a player cares about, differing in capacity, which is a number rather than a shape. Four meshes would have been eight more batches on a frame with two to spare. **The cargo half is not built**: `FLAG_CARRYING` and a 0..1 fullness cross the bridge and `InstanceBatch` carries a per-instance `shade`, so a laden hull is a shade change and no extra draw call — the wiring in `pushEntities` is what remains |
