...
| P2-T18 | Perf: the Phase 2 scene and its gate | PARTIAL | P2-T04, P2-T06 | 300 buildings across all 29 types plus 200 units; draw calls asserted | **The draw-call half is done and it PROVES ADR-0013 rather than restating it: 28 building batches for a 300-building base covering all 29 types, exactly the ceiling.** Asserted against the RecordingRenderer, not the perf harness — a draw-call count is a structural fact, identical on every machine, so it belongs in the fast suite. The first run reported 33 and looked like an architectural failure; it was the test counting unit batches against a buildings-only budget, so the two are now asserted separately (buildings ≤ 28, whole frame ≤ 36, measured 34). **Frame TIME at 300 buildings is not yet measured** — the perf scene still runs the MVP's 80 |
