...
describe("dismissal outlives the tab (P5-T10)", () => {
  ...
  it("does not read a dismissal out of somebody else's key, or write over one", () => {
    ...
    saveSettings({ tierOverride: "T1", edgeScroll: false });
    ...
    expect(loadSettings(), "dismissing the card destroyed the player's settings")
      .toEqual({ tierOverride: "T1", edgeScroll: false });
  });
...
describe("the card names controls the app actually has (P5-T10)", () => {
  ...
  beforeEach(() => {
    restore = stubCanvas();
    game = new Game(elements(), new RecordingRenderer(), "T0", { tierOverride: null, edgeScroll: false },
...
const T2: Settings = { tierOverride: "T2", edgeScroll: true };
...
describe("the settings model (P5-T10)", () => {
  ...
  it("marks exactly one option per row as active, and it is the one in force", () => {
    ...
    const auto = settingsModel({ settings: { tierOverride: null, edgeScroll: true }, currentTier: "T1" });
  ...
  it("separates the tier a player CHOSE from the tier that is running", () => {
    ...
    const dropped = settingsModel({ settings: { tierOverride: "T2", edgeScroll: true }, currentTier: "T0" });
  ...
  it("carries a whole Settings, never a patch", () => {
    ...
    const model = settingsModel({ settings: { tierOverride: "T3", edgeScroll: false }, currentTier: "T3" });
    ...
    expect(off.settings, "changing edge scrolling dropped the tier override")
      .toEqual({ tierOverride: "T3", edgeScroll: true });
  });
  ...
  it("still applies a choice in a browser that stores nothing", () => {
    ...
    throwingStorage();
    const chosen = settingsModel({ settings: { tierOverride: null, edgeScroll: true }, currentTier: "T0" })
    ...
    expect(loadSettings(), "a blocked store did not fall back to the defaults")
      .toEqual({ tierOverride: null, edgeScroll: true });
